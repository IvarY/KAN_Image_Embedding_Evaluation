...
KAN_SRC_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'kan_repos', 'Wav-KAN')
)
if KAN_SRC_PATH not in sys.path:
    sys.path.insert(0, KAN_SRC_PATH)

import KAN as WavKan

class WavKANModel(nn.Module):
    def __init__(self, feature_extractor=None,
                 emb_dim=256, n_class=10,
                 use_dropout=False, dropout_rate=0.5,
                 KAN_params=None):
        super(WavKANModel, self).__init__()
        ...
        if KAN_params is None:
            KAN_params = {}
        self.emb = WavKan.KANLinear(feature_extractor.n_out,
                                    emb_dim,
                                    **KAN_params)
